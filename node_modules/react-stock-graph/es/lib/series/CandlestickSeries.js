...
function getWicksSVG(props, candleData) {
	...
	var wicks = candleData.map(function (each, idx) {
		var d = each.wick;
		return React.createElement("path", _extends({ key: idx,
...
function getCandlesSVG(props, candleData) {
	...
	var candles = candleData.map(function (d, idx) {
		if (d.width <= 1) return React.createElement("line", _extends({ className: d.className, key: idx,
			x1: d.x, y1: d.y, x2: d.x, y2: d.y + d.height,
			stroke: d.fill
		}, candleProps));else if (d.height === 0) return React.createElement("line", _extends({ key: idx,
			x1: d.x, y1: d.y, x2: d.x + d.width, y2: d.y + d.height,
			stroke: d.fill
		}, candleProps));
		return React.createElement("rect", _extends({ key: idx, className: d.className,
			fillOpacity: opacity,
